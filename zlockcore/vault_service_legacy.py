...
class VaultService:
    ...
    def recover_master(self, path: Path, recovery: str) -> bytes:
        metadata = self.metadata(path)
        if not metadata.get('recovery_enabled'):
    ...
    def configure_totp(self, path: Path, master_key: bytes, mode: str, issuer: str, account: str) -> tuple[dict, str, str]:
        metadata = self.metadata(path)
